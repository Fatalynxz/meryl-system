...
export function PortalProfileSettingsModal({
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (!cleanOtp || cleanOtp.length !== 6) {
      setSecurityError("Please enter the complete 6-digit OTP code.");
      return;
    }
            ...
            {securityStep === "request" && (
              <form onSubmit={handleSendOtp} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-[11px] text-zinc-400">
                    A 6-digit One-Time Password (OTP) will be sent to this email to authenticate password reset.
                  </p>
                </div>
            ...
            {securityStep === "verify" && (
              <form onSubmit={handleVerifyOtp} className="space-y-4">
                ...
                <div className="flex flex-col gap-2 pt-2">
                  <Button
                    type="submit"
                    disabled={isVerifyingOtp || resetOtp.length !== 6 || newPassword.length < 8 || !confirmPassword}
